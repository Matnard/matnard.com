import { EventDispatcher, Matrix4, Vector3 } from "three";
...
const GamepadControls = function(object) {
    ...
    if (!gamepadSupportAvailable) {
      ...
      if ("ongamepadconnected" in window) {
        window.addEventListener(
          "gamepadconnected",
          window.onGamepadConnect.bind(this),
          false
        );
        window.addEventListener(
          "gamepaddisconnected",
          window.gamepadSupport.onGamepadDisconnect.bind(this),
          false
        );
      } else {
        this.startPolling();
  ...
  this.pollGamepads = function() {
    ...
    if (rawGamepads && rawGamepads[0]) {
      ...
      this.lon += this.filter(g.axes[2]);
      this.lat -= this.filter(g.axes[3]);
